...
app = FastAPI(title="سيادة Chat Engine", version="0.1.0")
...
async def get_token() -> str:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.post(f"{AP_BASE}/api/v1/authentication/sign-in", json={"email": AP_EMAIL, "password": AP_PASSWORD})
...
@app.get("/")
async def root():
    return {"status": "سيادة Chat Engine is running", "ap_base": AP_BASE}
...
async def get_connections():
    ...
    for c in data.get("data", []):
        connections.append({"name": c.get("displayName", c.get("pieceName", "?")), "pieceName": c.get("pieceName"), "externalId": c.get("externalId"), "status": c.get("status")})
    ...
    for name, action in data.get("actions", {}).items():
        ...
        for pname, pval in action.get("props", {}).items():
            if isinstance(pval, dict):
                props[pname] = {"type": pval.get("type", "?"), "required": pval.get("required", False), "displayName": pval.get("displayName", pname)}
...
async def list_flows():
    ...
    for f in data.get("data", []):
        flows.append({"id": f["id"], "displayName": f.get("version", {}).get("displayName", "?"), "status": f.get("status"), "updatedAt": f.get("updated")})
    return {"flows": flows}
...
async def list_runs(limit: int = 5):
    ...
    for r in data.get("data", []):
        runs.append({"id": r["id"], "flowId": r.get("flowId"), "status": r.get("status"), "duration": r.get("duration"), "created": r.get("created")})
    return {"runs": runs}
...
async def build_flow(req: BuildFlowRequest):
    ...
    await ap_request("POST", f"/api/v1/flows/{flow_id}", {"type": "IMPORT_FLOW", "request": {"displayName": req.display_name, "trigger": req.trigger_tree}})
...
async def test_flow(req: TestFlowRequest):
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.post(f"{AP_BASE}/api/v1/webhooks/{req.flow_id}/sync", json=req.test_data, headers={"Content-Type": "application/json"})
    ...
    return {"webhook_response": webhook_result, "latest_run": {"id": latest_run.get("id"), "status": latest_run.get("status"), "duration": latest_run.get("duration"), "flowId": latest_run.get("flowId")}}

@app.post("/api/test-sheets-format")
async def test_sheets_format(req: SheetsTestRequest):
    trigger_tree = {"name": "trigger", "type": "PIECE_TRIGGER", "displayName": "Webhook — Sheets Test", "valid": True, "settings": {"pieceName": "@activepieces/piece-webhook", "pieceVersion": "~0.1.1", "pieceType": "OFFICIAL", "packageType": "REGISTRY", "triggerName": "catch_webhook", "input": {"authType": "none"}, "inputUiInfo": {}}, "nextAction": {"name": "step_1", "type": "PIECE", "displayName": "Sheets insert", "valid": True, "settings": {"pieceName": "@activepieces/piece-google-sheets", "pieceVersion": "~0.9.5", "pieceType": "OFFICIAL", "packageType": "REGISTRY", "actionName": "insert_row", "input": {"auth": f"{{{{connections['{CONN_SHEETS}']}}}}", "spreadsheet_id": req.spreadsheet_id, "sheet_id": 0, "first_row_headers": True, "values": {"A": "{{trigger['body']['name']}}", "B": "{{trigger['body']['email']}}", "C": "{{trigger['body']['phone']}}"}}, "inputUiInfo": {}, "errorHandlingOptions": {"continueOnFailure": {"value": False}, "retryOnFailure": {"value": False}}}, "nextAction": {"name": "step_2", "type": "PIECE", "displayName": "Gmail confirm", "valid": True, "settings": {"pieceName": "@activepieces/piece-gmail", "pieceVersion": "~0.11.4", "pieceType": "OFFICIAL", "packageType": "REGISTRY", "actionName": "send_email", "input": {"auth": f"{{{{connections['{CONN_GMAIL}']}}}}", "receiver": ["a@siyadah-ai.com"], "subject": "Sheets Test SUCCEEDED", "body_type": "plain_text", "body": "Name: {{trigger['body']['name']}}\nEmail: {{trigger['body']['email']}}\nPhone: {{trigger['body']['phone']}}", "draft": False}, "inputUiInfo": {}, "errorHandlingOptions": {"continueOnFailure": {"value": False}, "retryOnFailure": {"value": False}}}, "nextAction": None}}}
    ...
    return {"build": build_result, "test": test_result, "format_used": {"A": "string", "B": "string", "C": "string"}, "verdict": "SUCCEEDED" if test_result.get("latest_run", {}).get("status") == "SUCCEEDED" else "CHECK"}
...
async def serve_chat():
    for d in [os.path.join(os.path.dirname(__file__), "static"), os.path.join(os.path.dirname(__file__), "..", "frontend")]:
